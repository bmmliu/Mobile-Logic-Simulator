    ...
    public void addBotViewEventListeners() {
        ...
        inputpin.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                TextComponent p_delay = new TextComponent().label("p_delay");
                dlg.add(p_delay);

                Picker pDelayPicker = new Picker();
                ...
                pDelayPicker.addActionListener((event1)->{
                    P_Delay = pDelayPicker.getSelectedString();
                });

                Command save = new Command("Save");

                dlg.add(pDelayPicker). add(ComponentGroup.enclose(new Button(save)));

                Command result = dlg.showStretched(BorderLayout.SOUTH, true);
                ToastBar.showMessage("P_Delay set to " + P_Delay + " ms", FontImage.MATERIAL_INFO);
